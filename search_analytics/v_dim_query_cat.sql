f-- a full and distinct list of search queries (SEO+PPC), with keyword category
...
paid_cat  as (select keyword, campaign_category from personal_space_db.abungsy_stg.v_ga_googleads_query_cat where campaign_category is not null ),
...
select fl.keyword
   ,  case
         when (campaign_category is null and page_category is null)
                                 and  fl.keyword LIKE ANY ('%worldremit%', '%world remi%','%word remit%','%world.remit%','%world remm%','%remit wor%','%workd rem%','wremit','world emit%','worldrmit%'
         ...
         when (campaign_category is null and page_category is null)
          ...
          when campaign_category is not null then campaign_category   -- paid campaign category takes precedence
          when page_category is not null then page_category           -- landing page categories
          end as category
  , visits_ppc, trx_ppc, clicks as clicks_seo, impressions as impressions_seo, tot_position
from full_list fl
...
-- check availability of categories 0.0389936458
select sum(case when category is null then clicks_seo end)/sum(clicks_seo) clicks_missing
...
from personal_space_db.abungsy_stg.v_query_cat

-- see summary by kw category
select category, sum(clicks_seo) clicks, sum(impressions_seo) impressions, count(distinct keyword) kw, sum(tot_position)/sum(impressions_seo) as position
from personal_space_db.abungsy_stg.v_query_cat
group by 1;

-- see biggest missing keywords
select keyword, clicks_seo from personal_space_db.abungsy_stg.v_query_cat  where category is null order by clicks_seo desc;
